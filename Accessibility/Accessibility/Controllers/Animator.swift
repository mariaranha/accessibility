...
//            card.log("ANIMATOR>> Begin dismiss transition.")
            ...
            let cardBackgroundFrame = detailVC.scrollView.convert(card.frame, to: nil)
            ...
            UIView.animate(withDuration: velocity, delay: 0, options: .curveEaseOut, animations: {
                ...
                self.card.layer.cornerRadius = CGFloat(self.card.cardRadius)
                
            }, completion: { _ in
                
                detailVC.layout(self.card.originalFrame, isPresenting: false, isAnimating: false)
                self.card.addSubview(detailVC.card)
...
//        card.log("ANIMATOR>> Begin present transition.")
        ...
        let bounceOffset = self.bounceTransform(card.originalFrame, to: card.frame)
